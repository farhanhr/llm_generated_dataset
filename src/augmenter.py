...
class TextAugmenter:
    ...
    def clean_llm_chatter(self, text):
        ...
        text = re.sub(r'^.*?(?:output|result|response|sms|message|hasil|berikut|here(?: is|s))[\w\s]*:\s*', '', text, flags=re.IGNORECASE)

        text = re.sub(r'\n\n.*?(?:hope this helps|semoga membantu|let me know).*$', '', text, flags=re.IGNORECASE | re.DOTALL)
